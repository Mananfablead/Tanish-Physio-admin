...
import { fetchBookings, updateBooking, deleteBooking, createBooking } from "@/features/bookings/bookingSlice";
...
export default function Bookings() {
  ...
  const { list: services, loading: servicesLoading, error: servicesError } = useSelector((state: any) => state.services);
  const { list: users, loading: usersLoading, error: usersError } = useSelector((state: any) => state.users);
  ...
  const handleUpdateBooking = async () => {
    ...
    try {
      await dispatch(updateBooking({ id: selectedBooking._id, bookingData: { ...bookingForm } }));
      setIsModalOpen(false);
      // Refresh the bookings list
      dispatch(fetchBookings());
    } catch (error) {
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {isEditing ? "Update the booking details." : "Enter the details for the new booking."}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                value={(() => {
                  ...
                  if (serviceIdValue && typeof serviceIdValue === 'object' && serviceIdValue.hasOwnProperty('_id')) {
