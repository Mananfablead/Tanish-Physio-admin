...
const ClinicMonitoring = ({
  ...
  const [monitorInfo, setMonitorInfo] = useState({ name: "", initials: "" });

  // Update monitor info when user prop changes
  useEffect(() => {
    if (user) {
      setMonitorInfo({
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-black">
        <div className="text-center text-white">
          ...
          <h2 className="text-2xl font-bold mb-2">Clinic Monitoring Session</h2>
          <p className="text-slate-500 mb-6">
            {clinicInfo.name} is in progress
          </p>
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <h1 className="text-white font-semibold tracking-tight">
              {clinicInfo.name} Session
            </h1>
            <p className="text-slate-500 text-xs mt-1">
              Session ID: {sessionId}
            </p>
            <p className="text-slate-500 text-xs">
              {monitorInfo.name} monitoring clinic session
            </p>
          </div>
        </div>
              ...
              {participants.length === 0 && (
                <>
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-300 font-semibold text-sm">
                      {clinicInfo.name?.charAt(0)?.toUpperCase() || "C"}
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <p className="text-white font-medium text-sm">
...
export default ClinicMonitoring;
